        ...
        Column(Modifier.fillMaxSize().padding(horizontal=24.dp, vertical=28.dp), verticalArrangement=Arrangement.SpaceBetween){
            Column(Modifier.padding(top=44.dp)){
                Row(verticalAlignment=Alignment.CenterVertically, horizontalArrangement=Arrangement.spacedBy(12.dp)){
                    ...
                    Column{
                        Text("LEDGER", style=MaterialTheme.typography.labelLarge, color=MaterialTheme.colorScheme.primary, fontWeight=FontWeight.Bold)
                        Text("Myanmar 2D", style=MaterialTheme.typography.titleLarge, fontWeight=FontWeight.Bold)
                    }
                }
                ...
                Row(horizontalArrangement=Arrangement.spacedBy(10.dp)){
                    WelcomeStat("Offline", "လုံခြုံ")
                    WelcomeStat("Fast", "လွယ်ကူ")
                    WelcomeStat("Clear", "ရှင်းလင်း")
                }
            }
...
@Composable fun AgentListScreen(vm:LedgerViewModel,onAgent:(Long)->Unit,onAdd:()->Unit,onEdit:(Long)->Unit,onWinning:()->Unit,onClosedDays:()->Unit,onSettings:()->Unit){
    ...
    AppScaffold("ဒိုင်များ", action={
        IconButton(onClick=onWinning){Icon(Icons.Default.EmojiEvents, "ထီပေါက်စဉ်")}
        IconButton(onClick=onClosedDays){Icon(Icons.Default.EventBusy, "ပိတ်ရက်")}
        ...
        LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding=PaddingValues(horizontal=16.dp, vertical=10.dp), verticalArrangement=Arrangement.spacedBy(14.dp)){
            item{Column(Modifier.padding(horizontal=4.dp)){Text("လုပ်ငန်းအနှစ်ချုပ်", style=MaterialTheme.typography.headlineSmall, fontWeight=FontWeight.Bold);Text("ဒိုင်များနှင့် စာရင်းများကို စီမံပါ", style=MaterialTheme.typography.bodyMedium, color=MaterialTheme.colorScheme.onSurfaceVariant)}}
            item{Surface(color=MaterialTheme.colorScheme.primaryContainer, shape=MaterialTheme.shapes.large){Row(Modifier.fillMaxWidth().padding(18.dp), verticalAlignment=Alignment.CenterVertically){Column(Modifier.weight(1f)){Text("ယနေ့အတွက် ပြင်ဆင်ထားပါ", style=MaterialTheme.typography.titleMedium, fontWeight=FontWeight.Bold);Text(if(days.isEmpty()) "ပိတ်ရက် မသတ်မှတ်ရသေးပါ" else "${days.size} ရက် ပိတ်ထားသည်", style=MaterialTheme.typography.bodySmall, color=MaterialTheme.colorScheme.onSurfaceVariant)};Icon(Icons.Default.CalendarToday, null, tint=MaterialTheme.colorScheme.primary)}}}
...
@Composable private fun Input(value:String,onValue:(String)->Unit,label:String,required:Boolean=false,keyboard:KeyboardType=KeyboardType.Text){val display=LocalLanguage.current.translate(label);OutlinedTextField(value,onValue,Modifier.fillMaxWidth(),label={Text(display+(if(required)" *" else ""))},singleLine=label!="မှတ်ချက်",keyboardOptions=KeyboardOptions(keyboardType=keyboard),shape=MaterialTheme.shapes.medium)}
@Composable private fun DateInput(value:String,onValue:(String)->Unit,label:String,enabled:Boolean=true){
...
@Composable fun AgentDetailScreen(vm: LedgerViewModel, id: Long, onBack: () -> Unit, onRoute: (String) -> Unit) {
    ...
    AppScaffold(profile.name,onBack,action={IconButton({onRoute("agentForm/$id")}){Icon(Icons.Default.Edit,"ပြင်ရန်")}}){p->
        LazyColumn(Modifier.fillMaxSize().padding(p),contentPadding=PaddingValues(horizontal=16.dp,vertical=10.dp),verticalArrangement=Arrangement.spacedBy(14.dp)){
            ...
            item{Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(10.dp)){Column(Modifier.weight(1f)){ActionTile(Icons.Default.Tune,"ကန့်သတ်ပမာဏ","limit/$id",onRoute)};Column(Modifier.weight(1f)){ActionTile(Icons.Default.MenuBook,"လမ်းညွှန်","format",onRoute)}}}
...
@Composable fun ClosedNumberScreen(vm:LedgerViewModel,agentId:Long,onBack:()->Unit){val numbers by vm.closedNumbers(agentId).collectAsState(initial=emptyList());var digit by rememberSaveable{mutableStateOf("")};AppScaffold("ပိတ်ဂဏန်း",onBack){p->Column(Modifier.fillMaxSize().padding(p).padding(AppDimens.screen)){Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){OutlinedTextField(digit,{digit=it.take(2)},Modifier.weight(1f),label={Text("ဂဏန်း 00–99")},keyboardOptions=KeyboardOptions(keyboardType=KeyboardType.Number));Button({vm.addClosedNumber(agentId,digit);digit=""},enabled=BetParser.validDigit(digit)){Text("သိမ်းမည်")}};Spacer(Modifier.height(20.dp));LazyColumn(verticalArrangement=Arrangement.spacedBy(8.dp)){if(numbers.isEmpty())item{Text("ပိတ်ထားသောဂဏန်း မသတ်မှတ်ရသေးပါ")}else items(numbers,key={it.id}){n->ListItem(headlineContent={Text(n.digit,fontWeight=FontWeight.Black)},supportingContent={Text("All customers under this Agent are blocked")},trailingContent={TextButton({vm.removeClosedNumber(n)}){Text("ဖယ်ရှားမည်")}},colors=ListItemDefaults.colors(containerColor=MaterialTheme.colorScheme.errorContainer))}}}}}
@Composable fun FormatScreen(onBack:()->Unit){val rows=listOf("ပါဝါ" to "05 50 16 61 27 72 38 83 49 94","နက္ခတ်" to "07 70 18 81 24 42 35 53 69 96","အပူး" to "00 11 22 33 44 55 66 77 88 99","ညီအကို" to "20 sibling numbers","အခွေ" to "345.100 → 34 43 45 54 35 53","အခွေပူး" to "အခွေ plus doubles","ပတ်သီး" to "9.100 → 19 unique numbers","ထိပ်စည်း" to "9.100 → 90…99","နောက်ပိတ်" to "9.100 → 09…99","R / reverse" to "10.20R100 → both directions");AppScaffold("Format",onBack){p->LazyColumn(Modifier.fillMaxSize().padding(p),contentPadding=PaddingValues(AppDimens.screen),verticalArrangement=Arrangement.spacedBy(10.dp)){item{Text("ထည့်သွင်းပုံ လမ်းညွှန်",style=MaterialTheme.typography.headlineSmall)};items(rows){(title,body)->OutlinedCard{Column(Modifier.padding(16.dp)){Text(title,style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.SemiBold);Text(body,color=MaterialTheme.colorScheme.onSurfaceVariant)}}}}}}
@Composable fun CommissionScreen(vm:LedgerViewModel,id:Long,onBack:()->Unit){val c by vm.customer(id).collectAsState(initial=null);var value by rememberSaveable{mutableStateOf("")};LaunchedEffect(c){c?.let{value=(it.commissionRateBasisPoints/100.0).toString().removeSuffix(".0")}};AppScaffold("ကော်မရှင်နှုန်းထား",onBack){p->FormColumn(p){Input(value,{value=it},"ရာခိုင်နှုန်း",true,KeyboardType.Decimal);Text("15 means 15%. Commission uses deterministic basis-point arithmetic.",color=MaterialTheme.colorScheme.onSurfaceVariant);FormActions(onBack,{vm.updateCommission(id,value,onBack)},value.toBigDecimalOrNull()?.let{it>=java.math.BigDecimal.ZERO&&it<=java.math.BigDecimal(100)}==true)}}}
@Composable fun LimitScreen(vm:LedgerViewModel,id:Long,onBack:()->Unit){val all by vm.allLimit(id).collectAsState(initial=null);val specials by vm.specialLimits(id).collectAsState(initial=emptyList());var allText by rememberSaveable{mutableStateOf("")};var digit by rememberSaveable{mutableStateOf("")};var amount by rememberSaveable{mutableStateOf("")};LaunchedEffect(all){allText=all?.amount?.toString()?:""};AppScaffold("ကန့်သတ်ပမာဏ",onBack){p->LazyColumn(Modifier.fillMaxSize().padding(p),contentPadding=PaddingValues(AppDimens.screen),verticalArrangement=Arrangement.spacedBy(12.dp)){item{Text("ကန့်သတ်ပမာဏ\nAll Limit",style=MaterialTheme.typography.titleLarge);Input(allText,{allText=it},"မထည့်ပါက အားလုံးအတွက် ကန့်သတ်ချက်မရှိပါ",keyboard=KeyboardType.Number);Button({vm.updateAllLimit(id,allText){}}){Text("သိမ်းမည်")}};item{HorizontalDivider();Text("ရွေးပြီးကန့်သတ်ပမာဏ\nSpecial Limit",style=MaterialTheme.typography.titleLarge);Input(digit,{digit=it.take(2)},"Digit",keyboard=KeyboardType.Number);Input(amount,{amount=it},"ပမာဏ",keyboard=KeyboardType.Number);Button({vm.addSpecialLimit(id,digit,amount);digit="";amount=""},enabled=BetParser.validDigit(digit)&&amount.toLongOrNull()?.let{it>0}==true){Text("သိမ်းမည်")}};items(specials){limit->Row(Modifier.fillMaxWidth().background(MaterialTheme.colorScheme.secondaryContainer,MaterialTheme.shapes.small).padding(14.dp),horizontalArrangement=Arrangement.SpaceBetween){Text("${limit.digit}  →  ${limit.amount.mmk()}");TextButton({vm.removeSpecialLimit(limit)}){Text("ဖယ်ရှားမည်")}}}}}}
@Composable
...
@Composable fun ClosedDayScreen(vm: LedgerViewModel, onBack: () -> Unit) {
    ...
    AppScaffold("ပိတ်ရက်", onBack) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(AppDimens.screen), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            DateInput(dateText, { dateText = it }, "ရက်စွဲ")
            Button(onClick = { runCatching { LocalDate.parse(dateText) }.getOrNull()?.let(vm::addClosedDay) }) { Text("သိမ်းမည်") }
            LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) { items(days, key = { it.id }) { day -> ListItem(headlineContent = { Text(day.date.displayDate()) }, trailingContent = { TextButton(onClick = { vm.removeClosedDay(day) }) { Text("ဖယ်ရှားမည်") } }) } }
        }
    }
}
