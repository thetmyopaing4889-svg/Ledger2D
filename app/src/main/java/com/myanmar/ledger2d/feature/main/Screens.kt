...
@Composable fun HomeScreen(vm:LedgerViewModel,onQuickEntry:()->Unit,onAgents:()->Unit,onWinning:()->Unit,onClosedDays:()->Unit,onSettings:()->Unit){
    val agents by vm.agents.collectAsStateWithLifecycle(); val days by vm.closedDays.collectAsStateWithLifecycle(); val today=LocalDate.now(); val closed=days.any{it.date==today}
    AppScaffold("မြန်မာ 2D\nယနေ့အလုပ်ခွင်",action={IconButton(onClick=onWinning){Icon(Icons.Default.EmojiEvents,"ထီပေါက်စဉ်")};IconButton(onClick=onSettings){Icon(Icons.Default.Settings,"ဆက်တင်များ")}}){p->
        LazyColumn(Modifier.fillMaxSize().padding(p),contentPadding=PaddingValues(horizontal=16.dp,vertical=12.dp),verticalArrangement=Arrangement.spacedBy(14.dp)){
            item{Column(Modifier.padding(horizontal=4.dp),verticalArrangement=Arrangement.spacedBy(4.dp)){Text("မင်္ဂလာပါ",style=MaterialTheme.typography.labelLarge,color=MaterialTheme.colorScheme.primary);Text("ယနေ့စာရင်းကို အမြန်စီမံပါ",style=MaterialTheme.typography.headlineSmall,fontWeight=FontWeight.Bold);Text(today.displayDate(),style=MaterialTheme.typography.bodyMedium,color=MaterialTheme.colorScheme.onSurfaceVariant)}}
            ...
            item{Button(onClick=onQuickEntry,enabled=!closed,modifier=Modifier.fillMaxWidth().height(58.dp),shape=MaterialTheme.shapes.medium){Icon(Icons.Default.AddCircle,null);Spacer(Modifier.width(10.dp));Text("အမြန်စာရင်းသွင်းရန်",style=MaterialTheme.typography.titleMedium)}}
            item{Text("အမြန်လုပ်ဆောင်ရန်",style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.Bold)}
            item{Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(10.dp)){HomeAction(Icons.Default.Store,"ဒိုင်များ",onAgents,Modifier.weight(1f));HomeAction(Icons.Default.EmojiEvents,"ပေါက်ဂဏန်း",onWinning,Modifier.weight(1f));HomeAction(Icons.Default.EventBusy,"ပိတ်ရက်",onClosedDays,Modifier.weight(1f))}}
...
@Composable fun QuickEntryScreen(vm:LedgerViewModel,onBet:(Long,Long)->Unit,onBack:()->Unit){
    val agents by vm.agents.collectAsStateWithLifecycle(); var agentId by rememberSaveable{mutableStateOf(0L)}; var customerId by rememberSaveable{mutableStateOf(0L)}; val customers by vm.customers(agentId).collectAsStateWithLifecycle(initialValue=emptyList())
    LaunchedEffect(agents){if(agentId==0L)agentId=agents.firstOrNull()?.id?:0L}; LaunchedEffect(customers){if(customers.none{it.id==customerId})customerId=customers.firstOrNull()?.id?:0L}
    AppScaffold("အမြန်စာရင်းသွင်းရန်",onBack){p->Column(Modifier.fillMaxSize().padding(p).padding(AppDimens.screen),verticalArrangement=Arrangement.spacedBy(16.dp)){Text("ဒိုင်နှင့် ထိုးသားကို ရွေးပြီး စာရင်းတင်ပါ",style=MaterialTheme.typography.bodyLarge,color=MaterialTheme.colorScheme.onSurfaceVariant);SelectionField("ဒိုင်ရွေးပါ",agents.firstOrNull{it.id==agentId}?.name?:"ဒိုင်မရှိသေးပါ",agents.map{it.id to "${it.name} • ${it.rate} ဆ"},agentId){agentId=it;customerId=0L};SelectionField("ထိုးသားရွေးပါ",customers.firstOrNull{it.id==customerId}?.name?:"ထိုးသားရွေးပါ",customers.map{it.id to it.name},customerId){customerId=it};Spacer(Modifier.weight(1f));Button(onClick={if(agentId>0&&customerId>0)onBet(agentId,customerId)},enabled=agentId>0&&customerId>0,modifier=Modifier.fillMaxWidth().height(56.dp),shape=MaterialTheme.shapes.medium){Text("အကွက်နှင့် ထိုးကြေးထည့်ရန်",style=MaterialTheme.typography.titleMedium);Spacer(Modifier.width(8.dp));Icon(Icons.AutoMirrored.Filled.ArrowForward,null)}}}
}
@Composable private fun SelectionField(label:String,selected:String,options:List<Pair<Long,String>>,value:Long,onSelect:(Long)->Unit){var expanded by remember{mutableStateOf(false)};ExposedDropdownMenuBox(expanded=expanded,onExpandedChange={expanded=!expanded}){OutlinedTextField(selected,{},Modifier.fillMaxWidth().menuAnchor(),readOnly=true,label={Text(label)},trailingIcon={ExposedDropdownMenuDefaults.TrailingIcon(expanded)},shape=MaterialTheme.shapes.medium);ExposedDropdownMenu(expanded=expanded,onDismissRequest={expanded=false}){options.forEach{(id,title)->DropdownMenuItem(text={Text(title)},onClick={onSelect(id);expanded=false})}}}}
