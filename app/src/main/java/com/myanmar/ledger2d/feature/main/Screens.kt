...
@Composable fun AgentListScreen(vm:LedgerViewModel,onAgent:(Long)->Unit,onAdd:()->Unit,onEdit:(Long)->Unit,onWinning:()->Unit,onClosedDays:()->Unit){
    ...
    AppScaffold("အေးဂျင့်များ",action={TextButton(onClick=onWinning){Text("ထီပေါက်စဉ်")};TextButton(onClick=onClosedDays){Text("ပိတ်ရက်")};},fab={ExtendedFloatingActionButton(onClick=onAdd){Text("+ အေးဂျင့်") }}){padding->
...
@Composable fun CustomerDetailScreen(vm: LedgerViewModel, id: Long, onBack: () -> Unit, onRoute: (String) -> Unit) {
    ...
    AppScaffold(profile.name,onBack,action={TextButton({onRoute("customerForm/${profile.agentId}/$id")}){Text("Edit")}}){padding->
        LazyColumn(Modifier.fillMaxSize().padding(padding),contentPadding=PaddingValues(AppDimens.screen),verticalArrangement=Arrangement.spacedBy(16.dp)){
            ...
            item{Text("လုပ်ငန်းဆောင်တာများ",style=MaterialTheme.typography.titleMedium,fontWeight=FontWeight.Bold)}
            item{ElevatedCard(onClick={onRoute("bet/${profile.agentId}/$id")},modifier=Modifier.fillMaxWidth(),colors=CardDefaults.elevatedCardColors(containerColor=MaterialTheme.colorScheme.primaryContainer),elevation=CardDefaults.elevatedCardElevation(defaultElevation=6.dp)){Row(Modifier.fillMaxWidth().padding(20.dp),verticalAlignment=Alignment.CenterVertically){Column(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(5.dp)){Text("စာရင်းသွင်းရန်",style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.Bold,color=MaterialTheme.colorScheme.onPrimaryContainer);Text("မနက်/ညနေ ထိုးကြေးစာရင်းထည့်ရန်",style=MaterialTheme.typography.bodyMedium,color=MaterialTheme.colorScheme.onPrimaryContainer.copy(alpha=.82f))};Text("→",style=MaterialTheme.typography.headlineMedium,color=MaterialTheme.colorScheme.primary)}}}
...
@Composable fun BettingScreen(vm:LedgerViewModel,agentId:Long,customerId:Long,onBack:()->Unit,entryId:Long=0L){val nextDraw=remember { DrawSchedule.nextDraw(LocalDateTime.now()) };var lateEntryEnabled by rememberSaveable(entryId){mutableStateOf(entryId>0L)};var dateText by rememberSaveable(entryId){mutableStateOf(nextDraw.date.toString())};var session by rememberSaveable(entryId){mutableStateOf(nextDraw.session)};var raw by rememberSaveable(entryId){mutableStateOf("")};var format by rememberSaveable{mutableStateOf(QuickFormat.MANUAL)};val existing by vm.betEntry(entryId).collectAsStateWithLifecycle(initialValue=null);val preview by vm.preview.collectAsStateWithLifecycle();val submit by vm.submit.collectAsStateWithLifecycle();val date=runCatching{LocalDate.parse(dateText)}.getOrNull();val entryUnlocked=entryId>0L||lateEntryEnabled;LaunchedEffect(existing){existing?.let{lateEntryEnabled=true;dateText=it.entry.drawDate.toString();session=it.entry.drawSession;raw=it.entry.sourceText}};LaunchedEffect(raw,format,date,session){date?.let{vm.refreshPreview(customerId,agentId,it,session,raw,format,entryId)}};LaunchedEffect(submit){if(submit is SubmitState.Success)onBack()};AppScaffold(if(entryId==0L)"စာရင်းသွင်းရန်" else "စာရင်းပြင်ရန်",onBack){p->Column(Modifier.fillMaxSize().padding(p)){LazyColumn(Modifier.weight(1f),contentPadding=PaddingValues(start=AppDimens.screen,top=AppDimens.screen,end=AppDimens.screen,bottom=28.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){item{DateInput(dateText,{dateText=it},"ရက်စွဲ",entryUnlocked);if(entryId==0L&&!lateEntryEnabled){Text("နောက်လာမည့် draw ကို default ရွေးထားပါသည်။ အချိန်ကျော် draw အတွက် အောက်ပါ special ခလုတ်ကို အရင်နှိပ်ပါ။",style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant);OutlinedButton(onClick={lateEntryEnabled=true},modifier=Modifier.fillMaxWidth()){Text("အချိန်ကျော်ပြီး စာရင်းသွင်းရန်")}}};item{Text("ထိုးမည့်အချိန်",style=MaterialTheme.typography.titleLarge);SingleChoiceSegmentedButtonRow(Modifier.fillMaxWidth()){DrawSession.entries.forEachIndexed{i,s->SegmentedButton(selected=session==s,onClick={session=s},enabled=entryUnlocked,shape=SegmentedButtonDefaults.itemShape(i,2)){Text(s.label)}}}};item{OutlinedTextField(raw,{raw=it},Modifier.fillMaxWidth().heightIn(min=130.dp),enabled=entryUnlocked,label={Text("စာရင်းထည့်ရန်")},placeholder={Text(if(format==QuickFormat.MANUAL)"ဥပမာ 10.13.14 100" else "ပုံစံအတိုင်း ထည့်ပါ")},shape=MaterialTheme.shapes.medium)};item{Text("အမြန်ထည့်သွင်းပုံများ",style=MaterialTheme.typography.titleMedium,fontWeight=FontWeight.Bold);FlowRow(horizontalArrangement=Arrangement.spacedBy(8.dp)){QuickFormat.entries.forEach{f->FilterChip(selected=format==f,onClick={format=f},enabled=entryUnlocked,label={Text(f.label)})}}};item{PreviewCard(preview)}};Surface(shadowElevation=10.dp){Button(onClick={date?.let{if(entryId==0L)vm.confirm(customerId,agentId,it,session,raw) else existing?.let{item->vm.editConfirm(item,it,session,raw)}}},modifier=Modifier.fillMaxWidth().padding(AppDimens.screen).height(54.dp),enabled=entryUnlocked&&preview.canConfirm&&submit !is SubmitState.Working){Text(if(submit is SubmitState.Working)"အတည်ပြုနေသည်…" else if(entryId==0L)"Confirm" else "Update")}}}}}
