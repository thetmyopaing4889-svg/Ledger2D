...
class LedgerViewModel(private val container: AppContainer):ViewModel(){
    ...
    fun deleteWinner(value:WinningNumberEntity){ viewModelScope.launch { runCatching { container.winners.delete(value) }.onSuccess { changed() } } }
    ...
    suspend fun allAgentSummaries(date:LocalDate,session:DrawSession,after:Boolean):List<ScopeSummary> = agents.value.mapNotNull { agent -> val report=agentReport(agent.id,date,session,after); if (report.calculation.totalBet==0L && !report.winnerAvailable) null else ScopeSummary(agent.id,agent.name,report.calculation.totalBet,report.calculation.winningStake,report.calculation.payout,report.calculation.profitLoss,report.calculation.commission) }
    suspend fun allCustomerSummaries(agentId:Long,date:LocalDate,session:DrawSession,after:Boolean):List<ScopeSummary> = container.customers.getForAgent(agentId).mapNotNull { customer -> val report=customerReport(customer.id,date,session,after); if (report.calculation.totalBet==0L && !report.winnerAvailable) null else ScopeSummary(customer.id,customer.name,report.calculation.totalBet,report.calculation.winningStake,report.calculation.payout,report.calculation.profitLoss,report.calculation.commission) }
