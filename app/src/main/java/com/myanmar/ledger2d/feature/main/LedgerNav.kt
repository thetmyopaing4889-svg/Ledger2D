...
@Composable fun LedgerNav(container:AppContainer,modifier:Modifier=Modifier){
    ...
    NavHost(nav,"home",modifier){
        ...
        composable("home"){HomeScreen(vm,onQuickEntry={nav.navigate("quickEntry")},onAgents={nav.navigate("agentDashboard")},onWinning={nav.navigate("winning")},onClosedDays={nav.navigate("closedDays")},onSettings={nav.navigate("settings")},onLedger={goTab("todayLedger")},onSettlement={goTab("settlement")},onAddAgent={nav.navigate("agentForm/0")},onAddCustomer={nav.navigate("addCustomerHome")},onAgentDashboard={nav.navigate("agentDashboard")},onCustomerDashboard={nav.navigate("customerDashboard")},onNavigate={route->when(route){"home"->goTab("home");"ledger"->goTab("todayLedger");"settlement"->goTab("settlement");"manage"->goTab("agentDashboard")}})}
        ...
        composable("allAgentFeature/{feature}",listOf(navArgument("feature"){type=NavType.StringType})){e->SimpleListScreen("ဒိုင်အားလုံး",{nav.popBackStack()},"ရွေးထားသော feature ကို Agent အားလုံး scope အတွက် ဆက်လက်တည်ဆောက်နေပါသည်",emptyList())}
        composable("customerDashboard"){CustomerDashboardScreen(vm,{nav.popBackStack()},{nav.navigate("addCustomerHome")},{nav.navigate("customerScope/$it")})}
