...
@Composable fun LedgerNav(container:AppContainer,modifier:Modifier=Modifier){
    ...
    NavHost(nav,"home",modifier){
        ...
        composable("agentDashboard"){AgentDashboardScreen(vm,{nav.popBackStack()},{nav.navigate("agent/$it")},{nav.navigate("agentForm/0")},{nav.navigate("agentWorkspace/$it")})}
        ...
        composable("customerDashboard"){CustomerDashboardScreen(vm,{nav.popBackStack()},{nav.navigate("customer/$it")},{nav.navigate("customerWorkspace/$it")})}
        composable("customerWorkspace/{feature}",listOf(navArgument("feature"){type=NavType.StringType})){e->CustomerFeatureWorkspaceScreen(vm,e.arguments!!.getString("feature") ?: "history"){nav.popBackStack()}}
