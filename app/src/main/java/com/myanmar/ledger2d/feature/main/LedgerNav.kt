...
@Composable fun LedgerNav(container:AppContainer,modifier:Modifier=Modifier){
    ...
    NavHost(nav,"home",modifier){
        ...
        composable("agentDashboard"){AgentDashboardScreen(vm,{nav.popBackStack()},{},{nav.navigate("agentForm/0")},{feature->when(feature){"agentList"->nav.navigate("agentList");else->nav.navigate("agentScope/$feature")}})}
        ...
        composable("customerDashboard"){CustomerDashboardScreen(vm,{nav.popBackStack()},{},{feature->when(feature){"customerList"->nav.navigate("customerList");else->nav.navigate("customerScope/$feature")}})}
        ...
        composable("agent/{id}",listOf(navArgument("id"){type=NavType.LongType})){e->AgentInformationScreen(vm,e.arguments!!.getLong("id"),{nav.popBackStack()}){nav.navigate("agentForm/$it")}}
        ...
        composable("customers/{agentId}",listOf(navArgument("agentId"){type=NavType.LongType})){CustomerListWorkspaceScreen(vm,{nav.popBackStack()},{nav.navigate("customerInfo/$it")},{agentId->nav.navigate("customerForm/$agentId/0")})}
        ...
        composable("customer/{id}",listOf(navArgument("id"){type=NavType.LongType})){e->CustomerInformationScreen(vm,e.arguments!!.getLong("id"),{nav.popBackStack()}){agentId,id->nav.navigate("customerForm/$agentId/$id")}}
