...
object DrawSchedule {
    val morningResultTime: LocalTime = LocalTime.of(12, 1)
    ...
    fun isWorkingDay(date: LocalDate, closedDays: Set<LocalDate> = emptySet()): Boolean = isWeekday(date) && date !in closedDays
    ...
    fun nextDraw(now: LocalDateTime, closedDays: Set<LocalDate> = emptySet()): DrawIdentity {
        var date = now.toLocalDate()
        if (!isWorkingDay(date, closedDays)) {
            do { date = date.plusDays(1) } while (!isWorkingDay(date, closedDays))
