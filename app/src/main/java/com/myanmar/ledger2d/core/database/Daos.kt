...
@Dao interface BetDao {
    ...
    @Transaction @Query("SELECT * FROM bet_entries WHERE customerId=:customerId AND drawDate=:date AND drawSession=:session ORDER BY id DESC") fun observeCustomerEntries(customerId: Long, date: LocalDate, session: DrawSession): Flow<List<BetEntryWithLines>>
    ...
    @Query("SELECT bl.digit AS digit, COALESCE(SUM(bl.amount),0) AS amount, COALESCE(SUM(CAST(be.commissionAmount * bl.amount / NULLIF((SELECT SUM(bl2.amount) FROM bet_lines bl2 WHERE bl2.betEntryId=be.id),0) AS INTEGER)),0) AS commission FROM bet_lines bl JOIN bet_entries be ON be.id=bl.betEntryId WHERE be.agentId=:agentId AND be.drawDate=:date AND be.drawSession=:session GROUP BY bl.digit ORDER BY bl.digit") fun observeAgentTotalsWithCommission(agentId: Long, date: LocalDate, session: DrawSession): Flow<List<DigitCommissionRow>>
    ...
    @Query("UPDATE bet_entries SET commissionRateBasisPoints=:basisPoints, commissionAmount=CAST((SELECT COALESCE(SUM(bl.amount),0) FROM bet_lines bl WHERE bl.betEntryId=bet_entries.id) * :basisPoints / 10000 AS INTEGER), updatedAt=:updatedAt WHERE customerId=:customerId") suspend fun updateCustomerCommission(customerId: Long, basisPoints: Int, updatedAt: Long)
