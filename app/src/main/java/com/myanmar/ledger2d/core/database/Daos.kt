...
@Dao interface BetDao {
    ...
    @Transaction @Query("SELECT * FROM bet_entries WHERE agentId=:agentId AND drawDate=:date AND drawSession=:session ORDER BY id DESC") fun observeAgentEntries(agentId: Long, date: LocalDate, session: DrawSession): Flow<List<BetEntryWithLines>>
    ...
    @Query("UPDATE bet_entries SET commissionRateBasisPoints=:basisPoints, commissionAmount=CAST((SELECT COALESCE(SUM(bl.amount),0) FROM bet_lines bl WHERE bl.betEntryId=bet_entries.id) * :basisPoints / 10000 AS INTEGER) WHERE customerId=:customerId") suspend fun recalculateCustomerCommission(customerId: Long, basisPoints: Int)
